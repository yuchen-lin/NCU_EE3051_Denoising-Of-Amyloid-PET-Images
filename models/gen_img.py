...
def generate_images(model, inp, tar, name, save_img_folder_name_png, save_img_folder_name_nii, mode):
    print("predicting image"+str(name)+"...")

    if mode == '3d':
        pre = model(inp, training=True)[0,:,:,:,0]
    elif mode == '2d':
        pre = []
        for i in range(90):
            pre.append(model(tf.expand_dims(inp[0,i], axis=0), training=True)[0])
        pre = np.array(pre)[:,:,:,0]
    ...
    # Save to nii
    out1 = sitk.GetImageFromArray(inp)
    out2 = sitk.GetImageFromArray(tar)
    out3 = sitk.GetImageFromArray(pre)
    ...
    for n, sl in enumerate(range(90)):
        ...
        outname = f'./{save_img_folder_name_png}/{name}_{str(sl).zfill(3)}.png'
